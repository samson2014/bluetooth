...
-(void)session:(MCSession *)session didReceiveData:(NSData *)data fromPeer:(MCPeerID *)peerID{
    ...
    NSString *tag1 = @"playthemusic";
    NSString *tag2 = @"changeVolume";
...
//                           };
    ...
    NSData *someData = [[arr objectAtIndex:1] dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *dict1 = @{@"data": someData,
                           @"peerID": peerID
                           };
    
    if ([[arr objectAtIndex:0] isEqual:tag1]) {
       
        [[NSNotificationCenter defaultCenter] postNotificationName:@"MusicDidReceiveDataNotification"
                                                            object:nil
                                                          userInfo:dict1];
    }else if([[arr objectAtIndex:0] isEqual:tag2]){
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ValumeDidReceiveDataNotification"
                                                            object:nil
                                                          userInfo:dict1];
    }else {
